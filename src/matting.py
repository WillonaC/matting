...
# include channels  rgb  alpha_c  alpha_k  direction  trimap
...
def read_data_from_file(data_dir,fname):
    ...
    trimap_dir = os.path.join(data_dir,"trimap/")
    ...
    fpath_direction = os.path.join(direction_dir, fname+'.mat')
    fpath_trimap = os.path.join(trimap_dir, fname)
    ...
    image_trimap = Image.open(fpath_trimap)  
...
#        data = data_rgb * data_rgb3 
    ...
    data = data_rgb
    input_elements=[]
    input_elements.append(data_rgb)
    input_elements.append(data_alpha_c.reshape(data_rgb.shape[0],data_rgb.shape[1],1))
    input_elements.append(data_alpha_k.reshape(data_rgb.shape[0],data_rgb.shape[1],1))
    input_elements.append(data_direction)
    input_elements.append(data_trimap.reshape(data_rgb.shape[0],data_rgb.shape[1],1))
    
    for i in range(1,len(channels_include)):
        if channels_include[i]:
            data = np.concatenate([data,input_elements[i]],axis=2)
...
#            print("loss: ",loss)
            ...
            # save results
            mp.imsave(result_dir+fname[0:-4]+'_rgb.png',rgb_img)
            mp.imsave(result_dir+fname[0:-4]+'_alpha.png',alpha_img)
            mp.imsave(result_dir+fname[0:-4]+'_alpha_c.png',alpha_c_img)
            mp.imsave(result_dir+fname[0:-4]+'_alpha_k.png',alpha_k_img)
            mp.imsave(result_dir+fname[0:-4]+saveName+'_res.png',res_img)
            mp.imsave(result_dir+fname[0:-4]+saveName+'_result.png',result_img)
